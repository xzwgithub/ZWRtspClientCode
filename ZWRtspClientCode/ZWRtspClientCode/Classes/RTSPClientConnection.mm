...
#pragma mark - 建议rtsp连接
//单例
+(instancetype)shareInstance
{
    static RTSPClientConnection * instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[RTSPClientConnection alloc] init];
    });
    return instance;
}
...
-(void)startRtspWithUrl:(NSString *)rtspURL
{
    ...
    // openURL(*env, "openRTSP", [rtspURL UTF8String]);
    
    RTSPClient* rtspClient = ourRTSPClient::createNew(*env, [rtspURL UTF8String], RTSP_CLIENT_VERBOSITY_LEVEL, "openRTSP");
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        shutdownStream(rtspClient);//发送关闭命令给服务器
    });
}
...
void DummySink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
    ...
    if(!strcmp(fSubsession.mediumName(), "video"))
    {
        ...
        {
            RTSPClientConnection * rtspC = [RTSPClientConnection shareInstance];
            if(rtspC.delegate)
            {
                [rtspC.delegate decodeNalu:fReceiveBuffer Size:frameSize];
            }
        }
    }
